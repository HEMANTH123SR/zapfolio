...
import { ClientLinkedinRedirectComponent } from "@/components/component/clientLinkedinUrlComponent";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import Link from "next/link";

const Themes = async () => {
  const user = await currentUser();
  const findingUserResponseDataJsonFormat = await fetch(
    `http://localhost:3000/api/find-user?userId=${user?.id}`,
  );
  const findingUserResponseData =
    await findingUserResponseDataJsonFormat.json();
  let userCanProccedStatus =
    findingUserResponseData.message === "user found" ? true : false;
  if (findingUserResponseData.message !== "user found") {
    const userCreatedResponse = await fetch(
    ...
    userCanProccedStatus = userCreatedResponse.status === 201 ? true : false;
  }
  if (userCanProccedStatus === false) {
    return <ClientLinkedinRedirectComponent />;
  } else {
    return (
